 ...
 *
 * Created by: Blake Douglas & Naim Shaqqou
 *
	...
	if (cur.type == varsym)
	{
		do {
			...
			if (level == 0)
				...
				addToSymbolTable(2, cur.name, 0, level, numVars + 2, 0);
...
void statement(lexeme *list)
{
	...
	if (cur.type == dosym)
	{
		...
		{
			// error 15
			if (cur.type == identsym || cur.type == dosym || cur.type == whensym || cur.type == whilesym || cur.type == readsym || cur.type == writesym ||cur.type == callsym)
			{
				printparseerror(15);
			} else { // error 16
				printparseerror(16);
	...
	// Checks ordering of when statement
	if (cur.type == whensym)
	{
		next(list);
		condition(list);
